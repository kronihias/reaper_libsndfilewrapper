@echo off
REM ============================================================================
REM  reaper_libsndfilewrapper - Windows build, codesign and installer pipeline.
REM
...
REM
REM  Output:
REM    _WIN_RELEASE\reaper_libsndfilewrapper_vX.Y.Z_win64_setup.exe -- installer
REM    that drops:
REM      %APPDATA%\REAPER\UserPlugins\reaper_libsndfilewrapper.dll
REM    (statically linked: libsndfile + WDL + static CRT — no external deps)
REM ============================================================================
...
echo === reaper_libsndfilewrapper v%VERSION% - Windows installer build ===
...
REM Locate the built DLL - CMake on MSVC writes to BUILD\Release\
set PLUGIN_BUILT=%BUILD_DIR%\Release\reaper_libsndfilewrapper.dll
if not exist "%PLUGIN_BUILT%" (
    for /r "%BUILD_DIR%" %%F in (reaper_libsndfilewrapper.dll) do set PLUGIN_BUILT=%%F
)
if not exist "%PLUGIN_BUILT%" (
    echo Error: build did not produce reaper_libsndfilewrapper.dll
    exit /b 1
)
...
copy /Y "%PLUGIN_BUILT%" "%INSTALL_PARENT%\reaper_libsndfilewrapper.dll" >nul

REM LGPL: ship the plugin's and libsndfile's license texts.
set LICENSE_DIR=%INSTALL_PARENT%\reaper_libsndfilewrapper-licenses
mkdir "%LICENSE_DIR%"
copy /Y "%ROOT%\COPYING" "%LICENSE_DIR%\COPYING.txt" >nul
copy /Y "%ROOT%\libsndfile\COPYING" "%LICENSE_DIR%\libsndfile-COPYING.txt" >nul
...
:sign_dlls
for %%F in ("%INSTALL_PARENT%\reaper_libsndfilewrapper.dll") do (
    echo signing %%F
...
REM ============================================================================
set ISS=%ROOT%\scripts\installer\reaper_libsndfilewrapper.iss
if not exist "%ISS%" (
